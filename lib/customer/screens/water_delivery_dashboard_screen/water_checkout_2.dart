...
class WaterCheckout2 extends ConsumerStatefulWidget {
  final Map<String, dynamic>? waterData;
  const WaterCheckout2({super.key, this.waterData});

  @override
  ConsumerState<WaterCheckout2> createState() => _WaterCheckout2State();
}

class _WaterCheckout2State extends ConsumerState<WaterCheckout2> {
  ...
  Widget build(BuildContext context) {
    ...
    _requestNumber ??= widget.waterData?['requestNumber']?.toString();
    _serviceRequestId ??= widget.waterData?['serviceRequestId']?.toString();
    _selectedProviderIndex = (widget.waterData?['selectedProviderIndex'] as int?) ?? _selectedProviderIndex;
    if (_selectedProviderIndex >= 0 && providersState.providers.isNotEmpty && _selectedProviderIndex < providersState.providers.length) {
      ...
      _amount = widget.waterData?['amount'] is num ? (widget.waterData!['amount'] as num).toDouble() : p.basePrice;
    }
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    const Text('Please select your mode of payment to proceed:', style: TextStyle(fontSize: 14, color: Color(0xFF1E1E1E))),
      ...
      child: Row(
        ...
        children: [
          Row(children: [
            ...
            const Text('Checkout', style: TextStyle(color: Color(0xFF086788), fontSize: 18, fontWeight: FontWeight.w500)),
          ]),
          Container(
      ...
      child: Row(
        children: [
          SizedBox(
            width: 95,
            height: 59,
            child: Image.asset('assets/images/water_tank.png', fit: BoxFit.contain),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 4),
                Text('Estimated arrival ${_selectedProviderIndex >= 0 && providersState.providers.isNotEmpty && _selectedProviderIndex < providersState.providers.length ? (providersState.providers[_selectedProviderIndex].estimatedArrival ?? '-') : '-'} mins', style: const TextStyle(fontSize: 10, color: Color(0xFF8F8F8F))),
                SizedBox(height: 6),
              ],
            ),
          ),
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () async {
          ...
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(ok ? 'Payment initiated' : 'Payment failed: ${res.message}')));
          if (ok) {
